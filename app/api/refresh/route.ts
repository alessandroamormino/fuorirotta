...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Refresh] Triggering n8n workflow for global data refresh:', cacheQuery)
...
export async function GET() {
  return NextResponse.json({
    ...
    usage: {
      method: 'POST',
      body: {
        cities: 'Array<string> (optional) - Cities to refresh, defaults to all Lombardy cities',
        dateFrom: 'string (optional) - Start date in YYYY-MM-DD format, defaults to today',
        dateTo: 'string (optional) - End date in YYYY-MM-DD format, defaults to end of year',
      ...
      examples: {
        ...
        specificCities: 'POST /api/refresh with {"cities": ["Milano", "Bergamo"]}'
      }
    }
  })
}
